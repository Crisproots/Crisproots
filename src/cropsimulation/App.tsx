...
import Enhanced3DCropSimulation from './components/Enhanced3DCropSimulation';
import AdvancedCropSimulation from './components/AdvancedCropSimulation';
import PlantPhysiologyMonitor from './components/PlantPhysiologyMonitor';
import AdvancedEnvironmentalEngine from './components/AdvancedEnvironmentalEngine';
import CropSimulationDashboard from './components/CropSimulationDashboard';
...
import RealDataIntegration from './components/RealDataIntegration';
import {
  getAllCrops,
  getCropInfo,
  getCurrentStage,
...
import weatherService, { SimulationWeatherParams, WeatherData } from './services/weatherService';
import geolocationService from './services/geolocationService';
import { SoilData } from './services/soilGridsService';
...
function App() {
  ...
  const [bbchByDay, setBbchByDay] = useState<Record<number, { code: string, stage: BBCHStage, modelState: unknown }>>({});
  const [maxDay, setMaxDay] = useState(0);
  const [coordinates, setCoordinates] = useState<{ latitude: number; longitude: number } | null>(null);
  const [soilSuitability, setSoilSuitability] = useState<number>(0);
  ...
  const handleStateChange = (locationString: string, country: string) => {
    ...
    // Extract coordinates if possible for real data integration
  ...
  const handleRealDataUpdate = (data: { weather: WeatherData; soil: SoilData | null; suitabilityScore: number }) => {
    // Update parameters with real weather data
    if (data.weather) {
      setParameters(prev => ({
        ...prev,
        temperature: data.weather.current.temp_c,
        humidity: data.weather.current.humidity,
        windSpeed: data.weather.current.wind_kph
      }));
    }

    // Update soil parameters with real data
    if (data.soil) {
      setParameters(prev => ({
        ...prev,
        soilPh: data.soil.pH,
        soilNitrogen: data.soil.nitrogen,
        soilOrganicMatter: data.soil.organicCarbon
      }));
    }

    setSoilSuitability(data.suitabilityScore);
  };
    ...
    <span className="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">Use My Location</span>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (!isNaN(lat) && coordinates) {
                              setCoordinates({ ...coordinates, latitude: lat });
                            } else if (!isNaN(lat)) {
                              setCoordinates({ latitude: lat, longitude: 77.2090 });
                            }
                          }}
                        />
                      </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (!isNaN(lon) && coordinates) {
                              setCoordinates({ ...coordinates, longitude: lon });
                            } else if (!isNaN(lon)) {
                              setCoordinates({ latitude: 28.6139, longitude: lon });
                            }
                          }}
                        />
                      </div>
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          alert('Failed to detect location. Please enter coordinates manually.');
                        }
                      }}
                    ...
                    {soilSuitability > 0 && (
                      <div className="mt-2 p-2 bg-green-100 rounded-lg">
                        ...
                        <div className={`text-xs ${
                          soilSuitability >= 80 ? 'text-green-700' :
                          soilSuitability >= 60 ? 'text-yellow-700' : 'text-red-700'
                        }`}>
                          {soilSuitability >= 80 ? 'Excellent for this crop' :
                           soilSuitability >= 60 ? 'Good for this crop' : 'May need soil improvements'}
                        </div>
                      </div>
                    )}
                  </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6 border-4 border-red-400">
            ...
            {isInitialized && (
              <>
                ...
                {coordinates && selectedCrop && (
                  <RealDataIntegration
                    latitude={coordinates.latitude}
                    longitude={coordinates.longitude}
                    selectedCrop={selectedCrop}
                    onDataUpdate={handleRealDataUpdate}
                  />
                )}
              </>
            )}
          ...
          {/* Center Panel - Advanced 3D Simulation Dashboard */}
          <div className="lg:col-span-2 space-y-6 border-4 border-green-400">
            <CropSimulationDashboard
              selectedCrop={selectedCrop}
              parameters={parameters}
              currentDay={currentDay}
              isRunning={isRunning}
              currentStage={currentStage ? {
                ...
                stage: {
                  name: currentStage.stage.description || 'Unknown Stage',
                  description: currentStage.stage.description || 'Stage description'
                }
              } : null}
            />
          </div>
